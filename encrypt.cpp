  ...
  if (input.fail()) {
    ...
    while (getline(input, line)) {
      ...
      CryptoPP::StreamTransformationFilter stfEncryptor(
          cbcEncryption, new CryptoPP::StringSink(cipherline));
      stfEncryptor.Put(reinterpret_cast<const unsigned char *>(line.c_str()),
                       line.length());
  ...
  if (input.fail()) {
    ...
    while (getline(input, cipherline)) {
      ...
      CryptoPP::StreamTransformationFilter stfDecryptor(
          cbcDecryption, new CryptoPP::StringSink(line));
      stfDecryptor.Put(
          reinterpret_cast<const unsigned char *>(cipherline.c_str()),
          cipherline.length());
